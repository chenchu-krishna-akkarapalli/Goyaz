...
export function NavMenu({ isOpen, onClose }: NavMenuProps) {
  ...
  // ── Level 2 → 3 mobile: sub-cat tap → cards overlay ──────────
  ...
  // ── Level 2 → 3 desktop: sub-cat click → Panel 3 slides in ───
  const [desktopSelectedSubCat, setDesktopSelectedSubCat] = useState<string | null>(null);
  ...
  function handleSectionSelect(nextSectionId: string) {
    // Reset desktop Panel 3 when switching sections
    setDesktopSelectedSubCat(null);
  ...
  // ── Level 2 → 3 desktop ───────────────────────────────────────
  function handleDesktopSubCatSelect(subCat: string) {
    setDesktopSelectedSubCat(subCat);
  }
  ...
  // Desktop: Panel 2 is always 240px when a list exists (cards go in Panel 3).
  // When only cards (no list), Panel 2 itself is 640px.
  const childPanelWidth = showCardsPanel && !showListPanel ? 640 : 240;

  // Desktop Panel 3: visible when a sub-cat is chosen and the section has cards
  const showDesktopPanel3 = showChildPanes && showListPanel && showCardsPanel && desktopSelectedSubCat !== null;

  // ── Shared cards content (desktop Panel 3 + mobile Level 3) ──
  ...
  return (
    <div className="fixed inset-0 z-[100] flex" aria-modal="true" role="dialog">
          ...
          <div className="nav-menu-scrollbar absolute inset-x-0 bottom-[118px] top-[88px] overflow-y-auto pr-4">
            <div className="pl-[20px] lg:pl-[40px] pr-4">
              <div className="flex w-full lg:w-[199px] flex-col gap-[100px] uppercase" style={futuraFont}>
                  ...
                  {NAV_MENU_SECTIONS.map((section, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between text-left uppercase transition-opacity duration-200 hover:opacity-60 ${isActive ? "text-black" : "text-[#4f4f4f]"}`}
                        style={itemStyle(index, 120, 55, isOpen, reduced)}
                      >
                        <span>{section.label}</span>
                        <span className={`transition-opacity duration-200 ${isActive ? "opacity-70" : "opacity-25"}`}>
                          <svg width="12" height="12" viewBox="0 0 1024 1024" fill="currentColor" aria-hidden>
                            <path d="M765.7 486.8L314.9 134.7A7.97 7.97 0 0 0 302 141v77.3c0 4.9 2.3 9.6 6.1 12.6l360 281.1-360 281.1c-3.9 3-6.1 7.7-6.1 12.6V883c0 6.7 7.7 10.4 12.9 6.3l450.8-352.1a31.96 31.96 0 0 0 0-50.4z" />
                          </svg>
                        </span>
                      </button>
                ...
                {/* Secondary links */}
                <div className="flex w-full lg:w-[122px] flex-col items-start gap-[30px] text-[16px] text-[#4f4f4f]">
      ...
      {/* ══════════════════════════════════════════════
          ...
          Mobile: full-screen overlay with drill-down
          Desktop: 240px column (or 640px if cards-only)
      ══════════════════════════════════════════════ */}
      {showChildPanes && activeSection && (
        <div
          key={`child-panel-${activeSection.id}-${mobileDrillExiting ? "exit" : "enter"}`}
          className={`font-sans ${mobileDrillOpen ? "flex" : "hidden"} lg:flex absolute lg:relative inset-0 lg:inset-auto h-screen overflow-hidden bg-white w-full lg:w-[var(--child-w)] lg:max-w-[calc(100vw-280px)] z-30 lg:z-auto border-r border-black/10 ${
            reduced
              ? ""
              : mobileDrillExiting
        ...
        >
          ...
          <div className="flex w-full h-full flex-col pt-[60px] lg:pt-0 overflow-y-auto lg:overflow-hidden nav-menu-scrollbar">
            ...
            {/* ── Sub-categories list ── */}
            {showListPanel && (
              <div className={`relative w-full lg:h-full overflow-visible lg:overflow-hidden ${reduced ? "" : ANIMATION_CLASSES.navChildListIn}`}>
                  ...
                  <div className="flex flex-col items-start gap-[16px] lg:gap-[20px] text-[15px] uppercase text-black" style={futuraFont}>
                    {activeSection.subCategories.map((item, index) => (
                      <span key={item.label} style={itemStyle(index, 220, 55, true, reduced, "navItemSlideRight")} className="w-full">
                        {/* ── Desktop ── */}
                        {showCardsPanel ? (
                          /* Has cards → clicking opens Panel 3 */
                          <button
                            type="button"
                            onClick={() => handleDesktopSubCatSelect(item.label)}
                            className={`hidden lg:flex items-center justify-between w-full text-left uppercase transition-opacity duration-200 hover:opacity-60 ${
                              desktopSelectedSubCat === item.label ? "opacity-100" : "opacity-70"
                            }`}
                          >
                            <span>{item.label}</span>
                            <span className={`transition-opacity duration-200 ${desktopSelectedSubCat === item.label ? "opacity-80" : "opacity-30"}`}>
                              <ArrowRight />
                            </span>
                          </button>
                        ) : (
                          /* No cards → navigate directly */
                          <Link
                            href={item.href}
                            onClick={onClose}
                            className="hidden lg:flex items-center justify-between w-full text-left uppercase transition-opacity duration-200 hover:opacity-60"
            ...
            {/* ── Cards (only when no list panel — cards-only sections) ──
                Desktop: shown in Panel 2 (640px) since no Panel 3 needed.
                Mobile: shown here always (no level-3 drill needed).       */}
            {showCardsPanel && !showListPanel && (
              <div className={`relative w-full lg:h-full lg:flex-1 overflow-visible lg:overflow-hidden ${reduced ? "" : ANIMATION_CLASSES.navChildCardsIn}`}>
            ...
            {/* Mobile CTA for cards-only sections */}
            {!showListPanel && (
              <Link
                href={activeSection.ctaHref}
                onClick={onClose}
          ...
          {/* ── Mobile Level 3: cards overlay after sub-cat tap ── */}
          {mobileLevel3Open && showCardsPanel && (
            ...
            >
              ...
              <div className="flex-none flex items-center justify-between bg-white px-[20px] h-[60px] border-b border-black/10">
                ...
                <span className="text-[14px] uppercase text-black font-medium" style={futuraFont}>{selectedSubCat}</span>
                <button type="button" onClick={onClose} className="cursor-pointer transition-opacity duration-200 hover:opacity-60" aria-label="Close">
                  <CloseIcon />
                </button>
              </div>
      ...
      {/* ══════════════════════════════════════════════
          PANEL 3 — Desktop only
          Slides in when a sub-cat is clicked in Panel 2.
          Width: 640px. Hidden on mobile (mobile uses level-3 overlay above).
      ══════════════════════════════════════════════ */}
      {showDesktopPanel3 && activeSection && (
        <div
          key={`desktop-p3-${activeSection.id}-${desktopSelectedSubCat}`}
          className={`hidden lg:flex relative h-screen w-[640px] max-w-[calc(100vw-520px)] overflow-hidden bg-white animate-[navChildPanelRise_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards]`}
        >
          <div className="relative w-full h-full">
            {/* Header: selected sub-cat label + CTA */}
            <div
              className="absolute top-[100px] left-0 right-0 px-[40px] flex items-center justify-between"
              style={contentSwapStyle(true, reduced, 80)}
            >
              <span className="font-sans text-[15px] uppercase text-black" style={futuraFont}>
                {desktopSelectedSubCat}
              </span>
              <Link
                href={activeSection.ctaHref}
                onClick={onClose}
