...
export function CurrentObsessionsSection() {
  return (
    <section className="flex flex-col gap-10 items-center w-full" data-mfe="section">
      ...
      {/* ── Category cards grid ── */}
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-10 w-full">
        {CURRENT_OBSESSIONS.map(({ label, img, objectPosition, href }, index) => {
          const card = (
            <div className={`flex flex-col gap-3 lg:gap-5 items-center w-full ${ANIMATION_CLASSES.hoverZoomBase}`}>
              <div className="border-[#083c30] border-[0.5px] border-solid aspect-square relative rounded-[20px] lg:rounded-[30px] w-full overflow-hidden">
                <img
                  alt={label}
                  className={`absolute inset-0 w-full h-full object-cover rounded-[30px] pointer-events-none ${ANIMATION_CLASSES.hoverZoomImg}`}
                  src={img}
